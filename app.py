...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        button { background: #a178ff; color: white; padding: 12px 20px; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; }
        button:disabled { background: #444; }
        .status { margin: 15px 0; color: #ffc107; display: none; }
        ...
        .download-btn { background: #28a745; color: white; text-decoration: none; padding: 6px 15px; border-radius: 5px; font-weight: bold; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div id="statusText" class="status">Bypassing restrictions... Please wait...</div>
        <div id="resultBox" class="result-box">
            <h3 style="color: #a178ff; margin-bottom: 10px;">Download Links Ready:</h3>
    ...
    <script>
        async function fetchLinks() {
            ...
            try {
                ...
                let check = false;
                if(data.video_url) {
                    list.innerHTML += `<li class="link-item"><span>🎥 Video (1080p HD)</span><a href="${data.video_url}" target="_blank" class="download-btn">Download</a></li>`;
                    check = true;
                }
                if(data.audio_url) {
                ...
                if(check) { resultBox.style.display = 'block'; } 
                else { alert('Server busy hai, please ek baar dobara click karein.'); }
                
            } catch(err) {
                statusText.style.display = 'none';
                btn.disabled = false;
                alert('Connection reset! Dobara try karo bhai.');
            }
        }
    </script>
</body>
...
# --- BACKEND (Flask + Alternate Global Node) ---
@app.route('/')
def index():
...
def get_links():
    try:
        ...
        # Yeh cobalt ka sub-node hai jo stable chal raha hai
        api_url = "https://bndb2.cobalt.tools/api/json"
        headers = {'Accept': 'application/json', 'Content-Type': 'application/json', 'User-Agent': 'Mozilla/5.0'}
        
        v_res = requests.post(api_url, json={"url": video_url, "videoQuality": "1080"}, headers=headers, timeout=10)
        v_data = v_res.json() if v_res.status_code == 200 else {}
        
        a_res = requests.post(api_url, json={"url": video_url, "isAudioOnly": True}, headers=headers, timeout=10)
        ...
        return jsonify({'error': f"Server busy, network switch kiya hai. Dobara try karein!"}), 500
